...
class Graph:
    ...
    def bfs_shortest_path_between(self, start_id: str, destination_id: str):
        ...
        if destination not in parents:
            print(f"{start.name} has no path to {destination.name}")
            return path
    ...
    def dijkstras_shortest_between(self, start_id: str, destination_id: str):
        ...
        print(f"Total weight: {dist[destination]}\n")


def timeit(fn, *args, **kwargs):
    t = time.time_ns()
    result = fn(*args, **kwargs)
    time_used = (time.time_ns() - t) / 10 ** 9
...
# Exercise 1-2 ===============================================
result, time_used = timeit(my_movie_graph.graph_metadta)
...
for id_1, id_2 in IDS_TO_CHECK:
    my_movie_graph.bfs_shortest_path_between(id_1, id_2)
time_used = (time.time_ns() - t) / 10 ** 9
...
t = time.time_ns()
for id_1, id_2 in IDS_TO_CHECK:
    my_movie_graph.dijkstras_shortest_between(id_1, id_2)
time_used = (time.time_ns() - t) / 10 ** 9
print(f"Time Used: {time_used} seconds\n{SEPARATOR}")
...
# Exercise 4 =================================================
result, time_used = timeit(my_movie_graph.component_size_to_count)
